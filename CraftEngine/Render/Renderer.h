...
namespace Craft
{
	...
	class CRAFT_API Renderer
	{
		...
		struct Frame
		{
			...
			//화면에 그릴 2차원 배열 문자값
			std::unique_ptr<CHAR_INFO[]> charInfoArray;	//선언만 해두고 정의는 생성자에서 make_unique 사용

			//그리기 정렬 값(2차원 배열)
			std::unique_ptr<int[]> sortingOrderArray;
		};

		//화면에 그릴 데이터를 명령 단위로 저장하기 위한 구조체, 내부에서만 사용
		struct RenderCommand
		{
			// 화면에 그릴 문자값
			std::string image;	//진짜 이미지는 아니지만 이미지라고 간주
	...
	public:
		...
		//Draw 이벤트 함수 -Engine에서 호출(엔진의 Draw와 다름)
		void Draw();
	...
	private:
		...
		//getter
		const ScreenBuffer* const GetCurrentBuffer() const;	//내부에서만 사용할 거라 private로 지정
