...
namespace Craft
{
	...
	void Renderer::Submit(const std::string& image, const Vector2& position, Color color, int sortingOrder)
	{
		...
		//그리는 데이터를 한번에 모아두면 최적화를 하기 좋다. (이 submit 형식으로 하면 좋다)
	...
	void Renderer::DrawRenderQueue()
	{
		...
		for (const RenderCommand& command : renderQueue)
		{
			...
			// 그리려는 문자열 길이 값, 읽기 전용 값이면 const를 붙여주자
			// 지금 액터는 x값만 계산, y값이 2라면 액터가 두 줄로 이루어져 있다는 것임
			const int length = static_cast<int>(command.image.length());

			// 글자의 시작 위치 <(A)> 라는 액터가 있다고 하면 이 액터의 시작 위치를 어디로 할 것인가? <가 시작위치로 설정할 것임
			const int startX = command.position.x;

			// 글자의 끝 위치 글자가 시작하는 인덱스에서 글자의 길이를 더하고 -1한 것이 마지막 글자 위치
			...
			//문자열 루프 순회하면서 글자를 2차원 배열에 하나씩 기록
			for (int x = visibleStart; x <= visibleEnd; ++x)
			{
				...
				//글자 2차원 배열의 인덱스
				//1차원 배열을 2차원처럼 쓸 때 쓰는 공식
				//(y*width) + x
				const int index = (command.position.y * screenSize.x) + x;	//x는 command.position.x을 보정한 것
	...
	void Renderer::Present()
	{
		...
		//(currentbufferIndex +1) % 2하는 방법도 있음
	}
