...
Enemy::Enemy(const std::string& image, int yPosition) : Actor(image)
{
    ...
    if (random % 2 == 0)
    {
        ...
        xPosition = static_cast<float>(Engine::Get().GetWidth() - width - 1);   //Engine::Get().GetWidth() - width하면 5가 되는데
    }
    else
    {
    ...
    //발사 타이머 사용 시간 설정 (1초에서 3초 사이의 시간을 랜덤으로)
...
void Enemy::Tick(float deltaTime)
{
    ...
    if (owner)
    {
        owner->SpawnActor<EnemyBullet>(bulletPosition, Util::RandomRange(10.0f, 20.0f));
    }
}
