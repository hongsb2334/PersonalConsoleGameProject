...
bool GameLevel::CanMove(const Vector2& playerPosition, const Vector2& nextPosition)
{
    ...
    //박스가 없으면 nullptr, nullptr이 있으면 박스가 있다.
    ...
    if (boxActor)
    {
        ...
        //1. 두 위치값이 있을 때 이동 방향 구하기(두 벡터 빼기)
        //플레이어 위치에서 다음 위치로 향하는 방향 구하기
        //필수 지식 - 벡터 개념
        //우리는 지금 (x, y)꼴로 정의했는데 사실 (x, y, 1), (x2, y2, 1)임, 둘이 빼면 (x-x2, y-y2, 0)이어서 벡터가 되는것임
        //(x,y,0), (x2, y2, 1)을 더하면 (x+x2, y+y2, 1)이 되어 위치가 됨
        Vector2 direction = nextPosition - playerPosition;
        //박스가 밀리는 위치 구하기
        //옵션 지식 - 그래픽스를 요구하는 곳에서는 필수
        //위치 + 벡터(크기, 방향을 가지는 데이터).
        //벡터는 위치 정보가 없음
        //하지만 이 덧셈은 수학적으로 정의되지 않음
        //원래는 안되는데 가능하도록 별도로 정의함
        //동차좌표계(위치, 벡터) | 아핀 변환
        // (x, y, w) : w가 0이면 벡터, 1이면 위치 , 3차원
        // (x, y, z, w) : w가 0이면 벡터, 1이면 위치, 4차원
        // (x1, y1, 1) + (x2, y2, 1)을 더하면 (x1+x2, y1+y2, 2)가 되는데 2는 정의되어 있지 않기 때문에 안됨(그래서 위치끼리 더하면 안된다, 하지만 위치끼리 빼는건 됨)
        ...
        for (const std::shared_ptr<Actor>& otherbox : boxList)
        {
            ...
            {
                //박스를 이동시킬 위치에 다른 박스가 있으면 이동불가
                return false;
            }
        }

        //박스가 밀리는 위치가 이동 가능한지 다시 확인
        for (const std::shared_ptr<Actor>& actor : actorList)
        {
            //박스가 밀리는 위치의 액터 검색
...
void GameLevel::LoadMap(const std::string& filename)
{
	...
	//파일 길이 확인 -> 파일 위치를 제일 뒤로 이동 시킨다음, 해당 위치 값 읽기
	fseek(file, 0, SEEK_END);
	long fileSize = ftell(file);

	//파일 제일 끝위치를 구한 다음에는 다시 처음으로 되돌리기
	//fseek(file, 0, SEEK_SET); //이거랑 같다
	rewind(file);
	...
	while (true)
	{
		...
		case 'p':
			//플레이어 액터 생성
			SpawnActor<Ground>(position);		//플레이어가 한 칸이동시, 원래 위치에는 Ground가 표시되어야 함.
			SpawnActor<Player>(position);
			break;

			//박스
		case 'b':
			SpawnActor<Ground>(position);		//박스가 한 칸이동시, 원래 위치에는 Ground가 표시되어야 함.
...
bool GameLevel::CheckGameClear()
{
    ...
    //목표 지점에 배치된 박스의 수가 타겟 수(목표 점수)와 같은 지 확인
